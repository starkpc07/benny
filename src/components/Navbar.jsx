...
import { 
  RiFireFill, RiUser3Fill, RiHome4Line, 
  RiCalendarEventLine, RiInformationLine, RiContactsLine 
} from 'react-icons/ri';
            ...
            >
              <div className="flex h-full flex-col justify-between p-6">
                ...
                <ul className="flex flex-col gap-2 sm:gap-4">
                  {navItems.map((item, i) => (
                    <motion.li key={item.name} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.05 }}>
                      <a 
                        href={item.path.startsWith('#') ? getNavLink(item.path) : item.path} 
                        onClick={item.name === 'Home' ? scrollToTop : () => setIsMenuOpen(false)}
                ...
                <div className="pb-2">
                  ...
                  >
                    {user?.photoURL ? (
                      <img src={user.photoURL} className="size-6 rounded-full object-cover" alt="" />
                    ) : (
                      <RiUser3Fill />
                    )} 
                    {user ? 'Account' : 'Login'}
      ...
      <header className="fixed inset-x-0 top-4 z-200 md:top-8">
        <div className="mx-4 max-w-6xl md:mx-auto">
          <nav className="relative flex h-14 items-center justify-between rounded-full bg-linear-to-r from-[#8B0000] via-[#FF8C00] to-[#8B0000] px-4 shadow-2xl md:h-20 md:px-8 border border-white/10">
            <Link to="/" onClick={scrollToTop} className="group relative z-210 flex items-center gap-3">
              <div className="grid size-9 place-items-center rounded-full bg-white/20 backdrop-blur-md md:size-12">
                <RiFireFill className="text-xl text-white md:text-2xl" />
              </div>
            ...
            <div className="relative z-210 flex items-center gap-2">
              ...
              >
                {user?.photoURL ? (
                  <img src={user.photoURL} className="size-full object-cover" alt="Profile" />
                ) : (
                  <RiUser3Fill />
                )}
              </Link>
