...
import {
  ...
  RiChat3Line,
  RiSendPlaneLine,
  RiMapPin2Line, // Added this icon
} from "react-icons/ri";

const Book = () => {
  const navigate = useNavigate();
  const [isSubmitting, setIsSubmitting] = useState(false);
  // 1. Added location to formData state
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // 2. Added eventLocation to the Firestore payload
      await addDoc(collection(db, "bookings"), {
        ...
        eventLocation: formData.location, // Synced here
    ...
    <div className="min-h-screen w-full bg-[#fafafa] flex flex-col items-center justify-center p-4 pt-10 md:pt-36 selection:bg-red-100">
      <div className="w-full max-w-200">
        <div className="hidden md:block mb-6 px-4">
          <Link
            to="/dashboard"
            className="flex items-center gap-2 text-[11px] font-black uppercase tracking-[0.2em] text-zinc-400 hover:text-zinc-900 transition-colors"
          >
        ...
        >
          ...
          <div className="w-full md:w-[60%] p-6 md:p-8">
            <div className="text-center mb-4">
              <h2 className="text-xl font-black uppercase tracking-[0.4em] text-zinc-900 mb-1">
                Inquiry
              </h2>
            ...
            <form className="space-y-3" onSubmit={handleSubmit}>
              ...
              {/* 3. Added Location Input Field */}
              <div className="space-y-1">
                <label className="text-[10px] uppercase font-black tracking-widest text-zinc-400 ml-1">
                  Event Location
                </label>
                <div className="relative">
                  <RiMapPin2Line className="absolute left-4 top-1/2 -translate-y-1/2 text-zinc-400" />
                  <input
                    type="text"
                    name="location"
                    required
              ...
              <div className="space-y-1">
                <label className="text-[10px] uppercase font-black tracking-widest text-zinc-400 ml-1">
                  Requirements (Optional) {/* Updated label for clarity */}
                </label>
                <div className="relative">
              ...
              <button
                ...
                className="w-full py-4 bg-linear-to-r from-[#8B0000] to-[#FF8C00] text-white hover:brightness-110 disabled:opacity-50 rounded-xl font-black uppercase text-[11px] tracking-[0.2em] transition-all active:scale-[0.98] flex justify-center items-center mt-4 shadow-lg"
              >
                {isSubmitting ? (
                  <div className="size-4 border-2 border-white border-t-transparent rounded-full animate-spin" />
                ) : (
                  <span className="flex items-center gap-2">
                    Confirm Booking <RiSendPlaneLine size={14} />
                  </span>
                )}
              </button>
            </form>
