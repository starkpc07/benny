...
const stallsData = [
  ...
  { id: 9, title: "Ice cream", image: iceImg }
];

const Stalls = () => {
  const [positionIndex, setPositionIndex] = useState(0);

  const handleNext = () => setPositionIndex((prev) => (prev + 1) % stallsData.length);
  const handlePrev = () => setPositionIndex((prev) => (prev - 1 + stallsData.length) % stallsData.length);
  ...
  const getCardVariant = (index) => {
    const diff = (index - positionIndex + stallsData.length) % stallsData.length;
      ...
      <div className="relative max-w-350 mx-auto flex items-center justify-center px-4 md:px-12">
        ...
        <div className="relative h-130 md:h-137.5 w-full flex flex-col items-center justify-center touch-pan-y">
          ...
          <div className="flex md:hidden gap-8 mt-16 pb-4">
            <button onClick={handlePrev} className="size-14 rounded-full bg-white border border-zinc-200 flex items-center justify-center shadow-lg active:scale-95 transition-transform">
              <RiArrowLeftSLine size={28} className="text-zinc-800" />
            </button>
            <button onClick={handleNext} className="size-14 rounded-full bg-white border border-zinc-200 flex items-center justify-center shadow-lg active:scale-95 transition-transform">
